...
{
	// TODO
	// Also, maybe this shouldn't be public?
}
			       ...
			       struct drgn_elf_file *file)
{
	...
	if (use_loaded && use_debug) {
		...
		drgn_log_info(prog, "using loadable file %s", file->path);
	} else if (use_debug) {
		drgn_log_info(prog, "using debug info file %s", file->path);
	}
...
#undef EXE_FORMAT
		...
		if (fd < 0) {
			drgn_log_debug(prog, "%s: %m", link_path);
			return NULL;
		}
		drgn_log_debug(prog, "found %s", link_path);
	} else if (module->kind == DRGN_MODULE_SHARED_LIBRARY) {
		snprintf(link_path, sizeof(link_path), MAP_FILES_FORMAT,
			 (long)prog->pid);
#undef MAP_FILES_FORMAT
		_cleanup_closedir_ DIR *dir = opendir(link_path);
		if (!dir) {
			if (errno != ENOENT) {
		...
		if (fd < 0) {
			drgn_log_debug(prog, "%s/%s: %m", link_path,
				       ent->d_name);
		}
		if (fd < 0)
			return NULL;
					     ...
					     struct drgn_module_try_files_state *state)
{
	...
	if (trying->debugaltlink_path[0] == '/' ||
		...
		if (!string_builder_appendn(&sb, trying->debug_file->path,
		    ...
		    !string_builder_null_terminate(&sb))
			return &drgn_enomem;
		err = drgn_module_try_file_internal(module, state, sb.str, -1,
						    true, NULL);
	}
	if (err || drgn_module_try_files_done(state))
		return err;
					 ...
					 long b, int result)
{
	...
	int fd = STDERR_FILENO; // TODO
	if (!drgn_log_is_enabled(prog, DRGN_LOG_WARNING)) // TODO
		return;
	...
	const bool tty = ioctl(fd, TIOCGWINSZ, &winsize) == 0;
	...
	if (!prog->dbinfo.debuginfod_have_url) {
		...
		if (url) {
			...
			// progress.
			if (tty) {
				write_all(fd, ansi_erase_line,
					  sizeof(ansi_erase_line) - 1);
			}
			drgn_log_debug(prog, "downloading from %s", url);
		}
#else
		prog->dbinfo.debuginfod_have_url = true;
#endif
	...
	if (done && result >= 0 && !prog->dbinfo.debuginfod_have_url) {
	...
	if (animate) {
		if (done) {
			...
			// get too short for the name and download size in small
			// terminals.
	...
	// "\r".
	write_all(fd,
		  sb.str + (orig_animate && !animate ? 1 : 0),
		  sb.len - (orig_animate && !animate ? 1 : 0));
}
...
#define try_debuginfod(which)							\
	do {									\
		...
		if (fd >= 0) {							\
			struct drgn_error *err =				\
				drgn_module_try_file_internal(module, state,	\
							      path, fd, false,	\
							      NULL);		\
			free(path);						\
			if (err)						\
...
out:
	if (!err && missing_debug_info_error_message.len > 0) {
		...
		string_builder_deinit(&missing_debug_info_error_message);
	}
