					  ...
					  PyObject *kwds)
{
	...
	static char *keywords[] = {"start", "end", NULL};
	struct index_arg start = {}, end = {};
	if (!PyArg_ParseTupleAndKeywords(args, kwds, "O&O&:set_address_range",
					 keywords, index_converter, &start,
					 index_converter, &end))
		return NULL;
	err = drgn_module_set_address_range(self->module, start.uvalue,
					    end.uvalue);
	if (err)
...
static PyObject *Module_get_start(Module *self, void *arg)
{
	uint64_t start, end;
	if (!drgn_module_address_range(self->module, &start, &end))
		Py_RETURN_NONE;
	return PyLong_FromUint64(start);
}

static PyObject *Module_get_end(Module *self, void *arg)
{
	uint64_t start, end;
	if (!drgn_module_address_range(self->module, &start, &end))
		Py_RETURN_NONE;
	return PyLong_FromUint64(end);
}
...
static PyMethodDef Module_methods[] = {
	{"set_address_range", (PyCFunction)Module_set_address_range,
	 METH_VARARGS | METH_KEYWORDS, drgn_Module_set_address_range_DOC},
